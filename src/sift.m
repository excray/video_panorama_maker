...
tpgm = sprintf('tmp_%d.pgm', system_dependent('getpid'));
tkey = sprintf('tmp_%d.key', system_dependent('getpid'));

%put these files in tmp folder
%%tpgm = strcat('../tmp/', tpgm);
%tkey = strcat('../tmp/', tkey);
...
command = [command strcat('<',tpgm) strcat(' >', tkey)]
eval(command);
